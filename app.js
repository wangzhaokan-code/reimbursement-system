...
function rows(file){ const raw=Array.isArray(file?.draft_slots)?file.draft_slots:[]; return Array.from({length:5},(_,i)=>raw[i]||{slot_index:i+1,expense_date:'',amount:''}); }
...
async function handleListAction(action,id){ const c=state.claims.find(x=>x.id===id); if(!c)return; if(action==='edit'){state.selectedClaim=c;state.mode='edit';state.message=null;render();} else if(action==='view'){state.selectedClaim=c;state.mode='detail';render();} else if(action==='delete'&&confirm(`确定删除草稿 ${c.claim_number}？文件数${stats(c).files}，收据数${stats(c).receipts}，总额${yen(stats(c).amount)}。`)){ const {error}=await supabase.from('reimbursement_claim').delete().eq('id',id); if(error){console.error(error);flash('删除失败，请刷新后重试','error');} else {await loadSession();flash('草稿已删除');} } }
function renderEditor(){ const c=state.selectedClaim; const subjects=allowedSubjects(); const subjectOptions=subjects.map(s=>`<option value="${s.id}" ${s.id===c.expense_subject_id?'selected':''}>${esc(s.display_name)}</option>`).join(''); const files=claimFiles(c); const fileHtml=files.map((f,fi)=>`<div class="file-card" data-file="${f.id}"><div class="file-head"><b>文件${fi+1}</b><span>${esc(f.original_filename||'')}</span><span class="muted">${statusLabel(f.upload_status||'draft')}</span><span>${f.upload_status==='ready'?'已上传':''}</span><button class="secondary" data-preview-file="${f.id}">查看凭证</button></div>${rows(f).map((r,ri)=>`<div class="slot ${partial(r)?'incomplete':''}"><span>第${ri+1}行</span><label>日期 <input type="date" data-file-id="${f.id}" data-slot-index="${ri+1}" data-slot-date value="${esc(r.expense_date||'')}"></label><label>金额（日元） <input type="number" min="0" step="1" data-file-id="${f.id}" data-slot-index="${ri+1}" data-slot-amount value="${esc(r.amount||'')}"></label><span>${partial(r)?'<b class="danger-text">请补全或清空</b>':''}</span></div>`).join('')}</div>`).join(''); const submit= c.id&&c.status==='returned'||c.id&&c.status==='draft'; app.innerHTML=`<section class="card"><div class="toolbar"><button id="back">返回我的报销</button><h1>${c.id?esc(c.claim_number):'新建报销'}</h1><span class="muted" id="saveState">${c.id?'已加载':'首次有效编辑时创建草稿'}</span></div>${messageHtml()}<label>费用归属主体 <select id="subject" ${c.id&&c.status!=='draft'?'disabled':''}>${subjectOptions||'<option>没有可用主体权限</option>'}</select></label><hr><label>上传凭证（每批最多5个文件） <input id="files" type="file" accept="image/jpeg,image/png,image/heic,application/pdf" multiple ${c.id&&c.status!=='draft'?'disabled':''}></label><div id="fileList">${fileHtml||'<p class="muted">尚未上传文件。</p>'}</div><div class="summary"><b>完整收据数：${stats(c).receipts}</b><b>总金额：${yen(stats(c).amount)}</b></div><div class="actions">${submit?'<button class="primary" id="submit">提交审核</button>':''}<button id="clear">清空本地编辑</button></div><div id="previewModal" class="preview-modal" hidden></div></section>`; bindEditor(); app.querySelectorAll('[data-preview-file]').forEach(b=>b.addEventListener('click',()=>previewEvidence(files.find(f=>f.id===b.dataset.previewFile)))); }
function bindEditor(){ document.querySelector('#back').onclick=async()=>{await new Promise(resolve=>setTimeout(resolve,500));await saveCurrent(false);state.mode='list';await loadSession();render();}; document.querySelector('#clear').onclick=()=>{for(const timer of slotSaveTimers.values())clearTimeout(timer);slotSaveTimers.clear();state.dirty=false;state.saving=false;state.selectedClaim={draft:true,expense_subject_id:preferredSubjectId(),evidence_file:[]};render();}; document.querySelector('#subject')?.addEventListener('change',async e=>{state.dirty=true;state.selectedClaim.expense_subject_id=e.target.value;await saveCurrent(true);renderEditor();}); document.querySelector('#files')?.addEventListener('change',onFiles); app.querySelectorAll('[data-slot-date],[data-slot-amount]').forEach(input=>input.addEventListener('input',e=>{const f=claimFiles(state.selectedClaim).find(x=>x.id===e.target.dataset.fileId); if(!f)return; const slotRows=rows(f); const r=slotRows[Number(e.target.dataset.slotIndex)-1]; if(e.target.hasAttribute('data-slot-date'))r.expense_date=e.target.value; else r.amount=e.target.value===''?'':Math.max(0,Math.trunc(Number(e.target.value))); f.draft_slots=slotRows;state.dirty=true;scheduleSlotSave(f);})); document.querySelector('#submit')?.addEventListener('click',submitCurrent); }
...
function downloadAdminReceiptsCsv(claims){const out=[['申请编号','主体','文件序号','收据序号','日期','金额']];claims.forEach(c=>claimFiles(c).forEach(f=>rows(f).filter(complete).forEach((r,i)=>out.push([c.claim_number,c.expense_subject?.display_name,f.file_order,i+1,r.expense_date.replaceAll('-',''),Number(r.amount)]))));downloadCsv('当前筛选收据明细.csv',out);}
...
supabase.auth.onAuthStateChange(async()=>{await loadSession();render();}); loadSession().then(render);
...
async function runVoid(claim){const reason=prompt('请输入作废原因');if(!reason?.trim()){flash('作废原因必填','error');return;}if(!confirm('确认作废该申请？文件、收据和审计记录将保留。'))return;try{await rpc('void_reimbursement_claim',{p_claim_id:claim.id,p_expected_version:claim.version,p_reason:reason.trim()});await loadSession();state.selectedClaim=state.claims.find(c=>c.id===claim.id)||claim;state.mode='detail';flash('申请已作废');}catch(e){flash(e.message,'error');}}
...
async function renderSystemAdmin(){
  ...
  const userRows=users.map(user=>{
    ...
    const summary=active.map(permission=>{const subject=state.subjects.find(item=>item.id===permission.expense_subject_id);return (subject?.display_name||'未知主体')+'：'+roleLabel(permission.role);}).join('、');
    const canEdit=platform||active.some(permission=>managedIds.has(permission.expense_subject_id)&&permission.role==='subject_admin');
    return '<tr><td>'+esc(user.short_name||user.full_name||user.display_name||'—')+'</td><td>'+esc(user.email||'—')+'</td><td>'+(user.status==='disabled'?'已停用':(user.is_platform_admin?'平台管理员':'已激活'))+'</td><td>'+esc(summary||'无主体权限')+'</td><td><button data-user-profile="'+user.id+'" '+(canEdit?'':'disabled')+'>编辑资料</button> <button data-user-edit="'+user.id+'" '+(canEdit?'':'disabled')+'>编辑权限</button>'+(platform&&!user.is_platform_admin&&user.status!=='disabled'?' <button class="danger" data-user-disable="'+user.id+'">停用</button>':'')+'</td></tr>';
  }).join('');
  ...
  const inviteRows=pendingEmails.map(email=>{const emailInvites=invites.filter(invite=>invite.normalized_email===email&&!invite.accepted_user_id&&invite.is_active);const first=emailInvites[0];return '<tr><td>'+esc(email)+'</td><td>'+esc(first?.short_name||first?.full_name||'—')+'</td><td>待首次登录</td><td>'+emailInvites.length+'项权限</td><td><button data-pending-edit="'+esc(email)+'">编辑权限</button> <button class="danger" data-pending-delete="'+esc(email)+'">删除</button></td></tr>';}).join('');
  ...
  app.querySelectorAll('[data-pending-edit]').forEach(button=>button.addEventListener('click',()=>{state.systemPermissionTarget={kind:'pending',email:button.dataset.pendingEdit,label:button.dataset.pendingEdit};renderSystemAdmin();}));
  app.querySelectorAll('[data-user-profile]').forEach(button=>button.addEventListener('click',async()=>{const user=users.find(item=>item.id===button.dataset.userProfile);if(!user)return;const shortName=prompt('人员简称',user.short_name||user.display_name||'');if(shortName===null)return;const fullName=prompt('人员姓名',user.full_name||user.display_name||'');if(fullName===null)return;try{if(platform)await rpc('manage_person_profile',{p_user_id:user.id,p_email:user.email,p_short_name:shortName,p_full_name:fullName,p_status:user.status||'active'});else{const subjectId=activeSubjectForUser(user,managedIds);await rpc('manage_subject_person',{p_user_id:user.id,p_email:user.email,p_short_name:shortName,p_full_name:fullName,p_expense_subject_id:subjectId,p_roles:activeRolesForSubject(user,subjectId),p_disable:false});}await loadSession();await renderSystemAdmin();flash('人员资料已保存');}catch(error){flash(error.message,'error');}});
...
function permissionMatrixHtml(target,perms,invites,visibleSubjects){
  ...
  const fields=target.kind==='pending'?'<div class="admin-form"><label>Google邮箱<input id="matrixEmail" type="email" value="'+esc(target.email||'')+'" placeholder="name@example.com" '+(target.isNew?'':'readonly')+' required></label><label>人员简称<input id="matrixShortName" value="'+esc(targetInvite?.short_name||'')+'" placeholder="例如 Leo" required></label><label>人员姓名<input id="matrixFullName" value="'+esc(targetInvite?.full_name||'')+'" placeholder="例如 LEO XXXX" required></label></div>':'';
