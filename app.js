...
async function reloadClaimVersion(claimId){
  await loadSession();
  return state.claims.find(claim=>claim.id===claimId)||null;
}

async function deleteDraft(claim){
  try{
    return await rpc('delete_reimbursement_draft',{p_claim_id:claim.id,p_expected_version:claim.version});
  }catch(error){
    if(!isVersionConflict(error))throw error;
    const latest=await reloadClaimVersion(claim.id);
    if(!latest||latest.status!=='draft')throw error;
    return await rpc('delete_reimbursement_draft',{p_claim_id:latest.id,p_expected_version:latest.version});
  }
}

function isVersionConflict(error){
  return error?.code==='40001'||String(error?.details||'').includes('VERSION_CONFLICT')||String(error?.serverMessage||'').includes('VERSION_CONFLICT');
}
...
function canMutateEvidence(claim){
  return Boolean(claim?.id&&claim.applicant_user_id===state.session?.user?.id&&['draft','returned'].includes(claim.status));
}
async function deleteEvidenceFile(claim,file){
  if(!canMutateEvidence(claim))return;
  ...
  try{
    ...
    await loadSession();
    state.selectedClaim=state.claims.find(item=>item.id===claim.id)||null;
    state.mode='edit';
    flash('凭证已删除');
  }catch(error){
    console.error('delete_reimbursement_evidence_file',error);
    flash(error.message,'error');
  }
}
async function replaceEvidenceFile(claim,file,input){
  const replacement=input.files?.[0];
  input.value='';
  if(!replacement||!canMutateEvidence(claim))return;
  const ext=(replacement.name.split('.').pop()||'').toLowerCase();
  ...
  try{
    ...
    const uploaded=await supabase.storage.from(bucket).upload(reserved.storage_path,replacement,{contentType:replacement.type||'application/octet-stream',upsert:false});
    if(uploaded.error)throw uploaded.error;
    await rpc('complete_evidence_file_upload',{p_evidence_file_id:reserved.evidence_file_id,p_actual_mime_type:replacement.type||'application/octet-stream',p_actual_file_size:replacement.size,p_actual_file_kind:kind});
    await loadSession();
    latest=state.claims.find(item=>item.id===claim.id)||latest;
    await rpc('replace_reimbursement_evidence_file',{p_claim_id:latest.id,p_target_evidence_file_id:file.id,p_replacement_evidence_file_id:reserved.evidence_file_id,p_expected_version:latest.version});
    await loadSession();
    state.selectedClaim=state.claims.find(item=>item.id===claim.id)||null;
    state.mode='edit';
    flash('凭证已替换，收据信息已保留');
  }catch(error){
    console.error('replace_reimbursement_evidence_file',error);
    flash('凭证替换失败，原凭证仍保留，请刷新后重试','error');
  }
}
function bindEvidenceActions(){
  const claim=state.selectedClaim;
  if(!canMutateEvidence(claim))return;
  claimFiles(claim).forEach(file=>{
